...
def _is_relu_wick_coef(nonlin_wick_coef: Callable) -> bool:
    ...
    this package being imported under more than one path (which would make the
    ``wick`` module objects distinct, hence distinct ``relu_wick_coef`` functions).
    """
    return (
...
def nonlin_kprop(
    ...
    Args:
        ...
        exact_relu_cov: For the ReLU nonlinearity at k_max==2 ONLY, use the EXACT
            bivariate-Gaussian ReLU covariance (the exact_relu_covariance module):
    ...
    if exact_relu_cov and k_max == 2 and _is_relu_wick_coef(nonlin_wick_coef):
        from .exact_relu_covariance import exact_relu_covariance_kprop
    ...
    if factor:
        ...
        if k_max == 3:
            from .factor_k3 import factored_nonlin_kprop_k3
            ...
            from .factor_k4 import factored_nonlin_kprop_k4
