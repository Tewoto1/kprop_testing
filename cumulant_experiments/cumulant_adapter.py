...
# Two runnable versions of the ReLU K=2 activation step, on the SAME code/model
# (no fork, no copy). Both only engage for ReLU AND k_max==2 (K>=3 is never
# affected):
#   - exact_relu_cov=False (default): the general harmonic "cumulant PROPAGATION"
#     algorithm. Its K=2 ReLU covariance uses the leading-order off-diagonal gain
#     Sigma_ij*c_i*c_j (c_i=Phi(mu_i/sigma_i)) -- i.e. an APPROXIMATION.
#   - exact_relu_cov=True: the EXACT bivariate Gaussian ReLU covariance -- the
#     off-diagonal is the true Cov(ReLU(Z_i), ReLU(Z_j)) with no gain
#     approximation (src.mlp_kprop.exact_relu_covariance; depends on scipy,
#     non-autograd). This is the "exact_relu_covariance" / "k2_exact_bivariate_relu"
#     / "exact_gaussian_relu_k2" path. Use it to test whether the EXACT covariance
#     calculation changes the trained-model behavior vs the approximate one.
